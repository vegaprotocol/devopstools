...
func (m *Manager) GetAssetWithName(name string) (*erc20token.ERC20Token, error) {
	...
	for _, asset := range m.assets {
		if strings.ToLower(asset.Name) == name {
			return m.GetAsset(asset.HexAddress, asset.EthNetwork)
		}
	}
...
func (m *Manager) GetAsset(hexAddress string, ethNetwork types.ETHNetwork) (*erc20token.ERC20Token, error) {
	client, err := m.ethClientManager.GetEthClient(ethNetwork)
	if err != nil {
		return nil, err
	}
	return erc20token.NewERC20Token(client, hexAddress)
}
