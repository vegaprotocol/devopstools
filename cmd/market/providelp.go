...
func RunProvideLP(args ProvideLPArgs) error {
	...
	for _, marketName := range []string{"AAPL", "AAVEDAI", "BTCUSD", "ETHBTC", "TSLA", "UNIDAI"} {
		market := getMarket(markets, proposerVegawallet.PublicKey, marketConfig[marketName].Marker)
		if market == nil {
			logger.Info("market does not exists", zap.String("market", marketName))
		} else {
			if err := provideLiquidity(marketName, network.DataNodeClient, lastBlockData, proposerVegawallet, args.Logger, market.Id); err != nil {
				failed = true
			}
		}
