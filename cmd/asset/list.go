...
func RunListAssets(args ListArgs) error {
	...
	signersWallets := []*wallet.EthWallet{}
	for _, node := range network.NodeSecrets {
		...
		signerWallet, err := wallet.NewEthWallet(ethClient, &secrets.EthereumWalletPrivate{
	...
	assetIDB32, err := tools.HexKeyToByte32(args.VegaAssetID)
	if err != nil {
		return err
	}
	...
	opts := network.NetworkMainWallet.GetTransactOpts()
...
func generateMultiSignature(signers []*wallet.EthWallet, msg []byte) ([]byte, error) {
